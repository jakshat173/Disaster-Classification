...
model = tf.keras.models.load_model("/Users/akshatjain/Desktop/CapstoneProject/DisasterCapstone10Classes/model_DenseNet121 (2).hdf5")

st.title("Artificial Intelligence Based Disaster Management System with 10 Classes")
...
if s == "Video":
    ...
    if f is None:
        ...
        st.write("P(no_disaster)",results[0]*100)
        st.write("P(Tsunami)",results[1]*100)
        st.write("P(thunder and lightning)",results[2]*100)
        st.write("P(Drought)",results[3]*100)
        st.write("P(Flood)",results[4]*100)
        st.write("P(Cyclone)",results[5]*100)
        st.write("P(Earthquake)",results[6]*100)
        st.write("P(Wildfire)",results[7]*100)
        st.write("P(Hurricane)",results[8]*100)
        st.write("P(Typhoon)",results[9]*100)
